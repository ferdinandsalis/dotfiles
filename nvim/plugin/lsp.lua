  ...
  return function(feature, commands)
    ...
    if not provider or client.server_capabilities[provider] then
      events[name].group_id =
        fss.augroup(fmt('LspCommands_%d_%s', bufnr, name), commands(provider))
...
local function setup_autocommands(client, bufnr)
  if not client then
    local msg =
      fmt('Unable to setup LSP autocommands, client for %d is missing', bufnr)
    ...
    return {
      {
        ...
        command = function(args)
          if
            not vim.g.formatting_disabled and not vim.b.formatting_disabled
          then
...
local function setup_mappings(_, bufnr)
  ...
  fss.nnoremap('gd', lsp.buf.definition, with_desc('lsp: definition'))
  fss.nnoremap('gr', lsp.buf.references, with_desc('lsp: references'))
  fss.nnoremap('K', lsp.buf.hover, with_desc('lsp: hover'))
  fss.nnoremap('gI', lsp.buf.incoming_calls, with_desc('lsp: incoming calls'))
  fss.nnoremap('gi', lsp.buf.implementation, with_desc('lsp: implementation'))
  fss.nnoremap(
    '<leader>gd',
    lsp.buf.type_definition,
    with_desc('lsp: go to type definition')
  )
  fss.nnoremap('<leader>cl', lsp.codelens.run, with_desc('lsp: run code lens'))
  fss.nnoremap('<leader>rn', lsp.buf.rename, with_desc('lsp: rename'))
end
...
do
  ...
  local function smart_quickfix_diagnostics()
    ...
    if not fss.is_vim_list_open() and id then
